...
void controller::prep_single_bf(){
  ...
  for(int i=0;i<p;i++){
	...
	fprintf(outfd, " %7.4e ", sslr.compute_log10_ABF(mcfg,phi2_vec[k], omg2_vec[k]));
      }
      fprintf(outfd,"\n");
    }
...
void controller::scan(){
  ...
  for(int i=0;i<p;i++){
	...
	continue;
      ...
      mcfg[i]=get_config(j);
      mcfg_map[i] = j;
      double rst = sslr.compute_log10_ABF(mcfg);
      abfv.push_back(rst);
      if(!meta)
    ...
    double log10_abf;
    if(!meta)
      log10_abf = sslr.log10_weighted_sum(abfv,config_wv);
    else
      log10_abf = abfv[0];

    fprintf(outfd,"%15s %10s   %9.3f          ",gene.c_str(), pars.geno_map[i].c_str(), log10_abf); 
...
void controller::gene_level_analysis(){
  ...
  for(int i=0;i<p;i++){
    ...
    double snp_abf;
    if(!meta)
      snp_abf = sslr.log10_weighted_sum(abfv,configwv);  
    else
      snp_abf = abfv[0];
  ...
  double gene_log10_abf = sslr.log10_weighted_sum(abf_vec,wts_vec);
...
void controller::run_mcmc(int burnin_in, int rep_in){
  ...
  while(counter<= total){
    ...
    double prop_lik = sslr.compute_log10_ABF(config);
   ...
   if(f<ratio){
	...
	//printf("new model %s: %f   %f\n",mid.c_str(),curr_lik, sslr.compute_log10_ABF(config));
      }
...
double* controller::build_marginal_proposal(map<int,int> &control_map){
  ...
  vector<vector<vector<double> > > Xgv = pars.geno_vec;
  ...
  for(int i=0;i<p;i++){
    ...
    if(control_map.find(i) != control_map.end()){
      for(int j=0;j<s;j++){
	vector<double> gvec = pars.geno_vec[j][i];
	Xcv[j].push_back(gvec);
      }
    }
  ...
  ssslr.set_sigma_option(0);
  ssslr.init(pars.pheno_vec,Xgv,Xcv);
  ssslr.set_effect_vec(phi2_vec,omg2_vec);
  ...
  for(int i=0;i<p;i++){
	...
	continue;
      mcfg[i]=get_config(j);
      double val = ssslr.compute_log10_ABF(mcfg);
      
      abf_vec.push_back(val);
      if(!meta)
    ...
    double snp_abf;
    if(!meta)
      snp_abf= ssslr.log10_weighted_sum(abf_vec,wts_vec);
    else
      snp_abf = abf_vec[0];
...
void controller::init_mcmc(){
  ...
  for(int i=0;i<p;i++){
	...
	gene_max_snp = i;
	gene_max_config = snp.max_config;
      }
    }
  ...
  */
  ...
  curr_lik = sslr.compute_log10_ABF(config);
...
int controller::propose_config(int index){
  ...
  if(draw < 0.3){
    int j = gsl_rng_uniform_int (r, totalc);
    if(meta){
      j = int(j/double(totalc));
    }
...
void controller::summarize_posterior(){
  ...
  for(int i=0;i<post_snp_vec.size();i++){
	...
	fprintf(outfd,"(c) %6.3f   ", snp_vec[index].abfv[j]);
    }
    fprintf(outfd,"\n");
  }
  ...
  if(post_r2){
    for(int i=0;i<s;i++){
      map< pair<int, int>, double> r2m;
      r2_table.push_back(r2m);
    }
...
void controller::compute_posterior_r2(){
  ...
  fprintf(outfd,"\nPosterior r^2\n");
  for(int i=0;i<p;i++){
    fprintf(outfd, "%15s   ",pars.geno_map[i].c_str());
    for(int j=0;j<s;j++){
...
double controller::compute_r2(int i1, int i2, int t){
  ...
  int n = pars.geno_vec[t][a].size();

  if(r2_table[t].find(index) == r2_table[t].end()){
    double *av = new double[n];
    double *bv = new double[n];

    for(int i=0;i<n;i++){
      av[i] = pars.geno_vec[t][a][i];
      bv[i] = pars.geno_vec[t][b][i];
      double cor = gsl_stats_correlation(av, 1, bv, 1, n);
      r2_table[t][index] = (cor*cor);
    }
    delete[] av;
    delete[] bv;
  }
...
double controller::get_conditional_prob(int index,vector<vector<int> >& mconfig, map<int,int>& mcfg_map){
  ...
  for(int i=0;i<totalc;i++){
    ...
    wcfg_map[index] = i;
    postw.push_back(sslr.compute_log10_ABF(wconfig) + compute_log10_prior(wcfg_map));
  }
